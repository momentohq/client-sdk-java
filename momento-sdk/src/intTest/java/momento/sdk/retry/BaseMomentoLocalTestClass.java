...
public class BaseMomentoLocalTestClass {
  ...
  protected final int port = Optional.ofNullable(System.getenv("TEST_ADMIN_PORT")).map(Integer::parseInt).orElse(8080);
  ...
  public static void withCacheAndCacheClient(
    ...
    int port = Optional.ofNullable(System.getenv("TEST_ADMIN_PORT")).map(Integer::parseInt).orElse(8080);
  ...
  public static void withCacheAndTopicClient(
    ...
    final String hostname =
        Optional.ofNullable(System.getenv("MOMENTO_HOSTNAME")).orElse("127.0.0.1");
    final int port =
            Optional.ofNullable(System.getenv("TEST_ADMIN_PORT")).map(Integer::parseInt).orElse(8080);
    final CredentialProvider credentialProvider = new MomentoLocalProvider(hostname, port);
