...
public final class CacheClientBuilder {
  ...
  // In normal course of operation, I do expect customers running production code to be able to just inject Momento in
  // in their application and then just always do get a Cache entity (since it is cheap) and then perform operations.
  //
  ...
  //  }
  //
  // We would never want customers to use createCacheIfDoesntExist. Although it ensures that Cache will always be
  // present, adding that option to production code will always result in a GET first before any other operation thus
  ...
  //
  // An intermediate way may be to keep this method and add good comments around why this shouldn't be done in
  // production code.
  //
  // Other option is to add this optional flag to get or set operations.
  //
  // So, we take this parameter on the builder but we only create a cache if we encounter CacheNotFound on the requested
  // operation. There is still some race so some calls may end up taking longer around the time cache doesn't exist. But
