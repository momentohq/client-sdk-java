...
public class MomentoBatchUtils {
    ...
    private static final int DEFAULT_IDLE_THREAD_KEEP_ALIVE_SECONDS = 60;
     ...
     */
    private MomentoBatchUtils(final CacheClient cacheClient,
    ...
    public static class MomentoBatchUtilsBuilder {
        ...
        /**
         * Sets the maximum number of concurrent requests allowed in a batch operation.
         * This limit is important for controlling the load on both the client and server side.
         * Increasing this number can lead to a higher network and server stress.
         * It is advised to keep this number reasonable and not exceed a certain threshold (e.g., 100),
         * as higher values may significantly impact the application's performance and stability,
         * particularly under constrained network conditions or limited server resources.
         *
         * @param maxConcurrentRequests The maximum number of concurrent requests. Caution should
         *                              be exercised if setting this value above 100.
     ...
     */
    public BatchGetResponse batchGet(final String cacheName,
     ...
     */
    public BatchGetResponse batchGet(final String cacheName,
                                     final BatchGetRequest.ByteArrayKeyBatchGetRequest request) {
        return batchGetInternal(cacheName, request.getKeys(), key -> new String(key, StandardCharsets.UTF_8));
    }
     ...
     * @return A CompletableFuture of BatchGetResponse.
     */
    private <T> BatchGetResponse batchGetInternal(final String cacheName,
                                                  final Collection<T> keys,
                                                  final Function<T, String> keyMapper) {

        if (keys.size() > this.maxConcurrentRequests) {
            return new BatchGetResponse.Error(CacheServiceExceptionMapper.convert(
                    new InvalidArgumentException(String.format("Number of keys should be less than " +
